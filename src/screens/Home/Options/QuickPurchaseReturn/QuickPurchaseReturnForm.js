...
const QuickPurchaseReturnForm = ({ navigation }) => {
  ...
  useEffect(() => {
    fetchVendorBillsOdoo({ limit: 100 }).then(data => setBills(data || [])).catch(() => {});
  ...
  const handleBillSelect = async (bill) => {
    ...
    try {
      // Load bill lines and auto-fill warehouse from PO in parallel
      const [billLines, poWarehouse] = await Promise.all([
