...
import { fetchQuickPurchaseReturnsOdoo } from '@api/services/generalApi';
...
const QuickPurchaseReturnListScreen = ({ navigation }) => {
  ...
  const { data, loading, fetchData, fetchMoreData } = useDataFetching(fetchQuickPurchaseReturnsOdoo);
  ...
  const renderItem = ({ item }) => {
    ...
    const billName = Array.isArray(item.source_invoice_id) ? item.source_invoice_id[1] : '-';
    ...
    return (
      <TouchableOpacity
        ...
        activeOpacity={0.7}
        onPress={() => navigation.navigate('QuickPurchaseReturnDetailScreen', { recordId: item.id })}
      >
        <View style={styles.row}>
          <Text style={styles.head} numberOfLines={1}>{item.name || `PR-${item.id}`}</Text>
        ...
        <View style={styles.row}>
          ...
          <Text style={styles.subContent} numberOfLines={1}>{billName}</Text>
        </View>
      </TouchableOpacity>
    );
  };
  ...
  return (
    <SafeAreaView>
      <NavigationHeader title="Purchase Returns" onBackPress={() => navigation.goBack()} />
      ...
      <RoundedContainer>
        {filteredData.length === 0 ? (
          <EmptyState imageSource={require('@assets/images/EmptyData/empty.png')} message="No Purchase Returns Found" />
        ) : (
          <FlashList
            data={formatData(filteredData, 1)} numColumns={1} renderItem={renderItem}
        ...
        <TouchableOpacity style={styles.fab} onPress={() => navigation.navigate('QuickPurchaseReturnForm')}>
