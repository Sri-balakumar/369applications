...
import { fetchQuickPurchaseReturnDetailOdoo, confirmQuickPurchaseReturnOdoo, cancelQuickPurchaseReturnOdoo, draftQuickPurchaseReturnOdoo } from '@api/services/generalApi';
...
const QuickPurchaseReturnDetailScreen = ({ navigation, route }) => {
  ...
  const fetchDetail = useCallback(async () => {
    if (!recordId) return;
    setLoading(true);
    try { setRecord(await fetchQuickPurchaseReturnDetailOdoo(recordId)); }
    catch (err) { console.error('[QuickPurchaseReturnDetail]', err); }
    ...
    Alert.alert('Confirm Return', 'This will create a Credit Note and Return Picking. Continue?', [
      ...
      { text: 'Confirm', onPress: async () => {
        ...
        try {
          ...
          await confirmQuickPurchaseReturnOdoo(recordId, companyId);
  ...
  const handleCancel = async () => {
    setActionLoading(true);
    try { await cancelQuickPurchaseReturnOdoo(recordId); Alert.alert('Cancelled', 'Return cancelled.', [{ text: 'OK', onPress: fetchDetail }]); }
  ...
  const handleSetDraft = async () => {
    setActionLoading(true);
    try { await draftQuickPurchaseReturnOdoo(recordId); Alert.alert('Done', 'Set back to draft.', [{ text: 'OK', onPress: fetchDetail }]); }
    catch (err) { Alert.alert('Error', err?.message || 'Failed.'); }
    finally { setActionLoading(false); }
  };

  if (!record) {
    return (<SafeAreaView><NavigationHeader title="Purchase Return" onBackPress={() => navigation.goBack()} /><OverlayLoader visible={true} /></SafeAreaView>);
  }

  const state = (record.state || 'draft').toLowerCase();
  const vendorName = Array.isArray(record.partner_id) ? record.partner_id[1] : '-';
  const billName = Array.isArray(record.source_invoice_id) ? record.source_invoice_id[1] : '-';
  ...
  return (
    <SafeAreaView>
      <NavigationHeader title={record.name || `PR-${record.id}`} onBackPress={() => navigation.goBack()} />
      <RoundedScrollContainer>
        ...
        <View style={styles.card}>
          <View style={styles.fieldRow}>
            <View style={styles.fieldCol}><Text style={styles.fieldLabel}>Vendor</Text><Text style={styles.fieldValue}>{vendorName}</Text></View>
            <View style={styles.fieldCol}><Text style={styles.fieldLabel}>Date</Text><Text style={styles.fieldValue}>{record.date || '-'}</Text></View>
          </View>
          <View style={styles.fieldRow}>
            <View style={styles.fieldCol}><Text style={styles.fieldLabel}>Source Bill</Text><Text style={styles.fieldValue}>{billName}</Text></View>
            ...
            return (
              <View key={line.id || idx} style={styles.lineItem}>
                ...
                <View style={styles.lineDetails}>
                  <Text style={styles.lineDetail}>Purchased: {line.purchased_qty || 0}</Text>
                ...
                <View style={styles.lineDetails}>
                  <Text style={styles.lineDetail}>Price: {currencySymbol} {(line.price_unit || 0).toFixed(2)}</Text>
                  <Text style={styles.lineSubtotal}>{currencySymbol} {(line.total || line.subtotal || 0).toFixed(2)}</Text>
                </View>
              </View>
            );
          })}
        ...
        <View style={styles.card}>
          <View style={styles.totalRow}><Text style={styles.totalLabel}>Untaxed:</Text><Text style={styles.totalValue}>{currencySymbol} {(record.amount_untaxed || 0).toFixed(2)}</Text></View>
          <View style={styles.totalRow}><Text style={styles.totalLabel}>Taxes:</Text><Text style={styles.totalValue}>{currencySymbol} {(record.amount_tax || 0).toFixed(2)}</Text></View>
          <View style={[styles.totalRow, styles.grandTotalRow]}><Text style={styles.grandTotalLabel}>Total:</Text><Text style={styles.grandTotalValue}>{currencySymbol} {(record.amount_total || 0).toFixed(2)}</Text></View>
        </View>
        ...
        {isDraft && (<><View style={{ marginVertical: 8 }}><Button backgroundColor={COLORS.primaryThemeColor} title="Confirm Return" onPress={handleConfirm} loading={actionLoading} /></View><View style={{ marginVertical: 4 }}><Button backgroundColor="#F44336" title="Cancel" onPress={handleCancel} loading={actionLoading} /></View></>)}
        {isCancelled && <View style={{ marginVertical: 8 }}><Button backgroundColor="#FF9800" title="Set to Draft" onPress={handleSetDraft} loading={actionLoading} /></View>}
      </RoundedScrollContainer>
