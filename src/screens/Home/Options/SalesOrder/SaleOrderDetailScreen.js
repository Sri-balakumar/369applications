...
const SaleOrderDetailScreen = ({ navigation, route }) => {
  ...
  const handleCreateInvoice = async () => {
    ...
    try {
      ...
      Alert.alert('Invoice Created', `Invoice has been created successfully.${result?.invoiceId ? `\nInvoice ID: ${result.invoiceId}` : ''}`, [
        { text: 'OK', onPress: () => fetchDetail() },
      ]);
    } catch (err) {
  ...
  const canInvoice = isConfirmed && invoiceStatus !== 'invoiced';
    ...
    <SafeAreaView>
      ...
      <RoundedScrollContainer>
        ...
        <View style={styles.statusRow}>
          {isConfirmed && invoiceStatus === 'invoiced' && (
            <View style={[styles.badge, { backgroundColor: '#009688', marginRight: 6 }]}>
              <Text style={styles.badgeText}>INVOICED ({invoiceCount})</Text>
            </View>
          )}
                  ...
                  <View key={line.id || idx} style={styles.editLineItem}>
                    ...
                    <View style={styles.editLineFields}>
                      ...
                      <View style={styles.editField}>
                        ...
                        <Text style={styles.lineSubtotal}>{currencySymbol} {(qty * price).toFixed(2)}</Text>
                      </View>
                    </View>
                  </View>
                );
