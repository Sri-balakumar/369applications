...
const QuickSalesReturnForm = ({ navigation }) => {
  ...
  useEffect(() => {
    fetchCustomerInvoicesWithReturnableFilterOdoo({ limit: 100 }).then(data => setInvoices(data || [])).catch(() => {});
  ...
  const handleInvoiceSelect = async (invoice) => {
    ...
    try {
      const [invoiceLines, soWarehouse, returnedQtys] = await Promise.all([
        ...
        fetchAlreadyReturnedSalesQtysOdoo(invoice.id),
      ]);
      setLines(invoiceLines.map(l => {
        const sold = l.quantity || 0;
        const alreadyReturned = returnedQtys[l.id] || 0;
        const returnable = Math.max(0, sold - alreadyReturned);
        return {
      ...
      showToastMessage('Failed to load invoice lines');
      setLines([]);
    } finally {
