...
const EstimateSaleDetailScreen = ({ navigation, route }) => {
  ...
  const handleCancel = async () => {
    ...
    catch (err) { Alert.alert('Error', err?.message || 'Failed.'); } finally { setActionLoading(false); }
  };
      ...
      <RoundedScrollContainer>
            ...
            return (
              <View key={line.id || idx} style={styles.lineItem}>
                ...
                <View style={styles.lineDetails}>
                  ...
                  <Text style={styles.lineSubtotal}>{currencySymbol} {(line.subtotal || 0).toFixed(2)}</Text>
                </View>
              </View>
            );
          })}
        ...
        <View style={styles.card}>
          <View style={[styles.totalRow, styles.grandTotalRow]}>
            <Text style={styles.grandTotalLabel}>Total (No Tax):</Text>
            <Text style={styles.grandTotalValue}>{currencySymbol} {total.toFixed ? total.toFixed(2) : '0.00'}</Text>
          </View>
          {state === 'done' && (
            <>
              <View style={styles.totalRow}><Text style={styles.totalLabel}>Paid:</Text><Text style={styles.totalValue}>{currencySymbol} {(record.amount_paid || 0).toFixed(2)}</Text></View>
              <View style={styles.totalRow}><Text style={styles.totalLabel}>Due:</Text><Text style={styles.totalValue}>{currencySymbol} {(record.amount_due || 0).toFixed(2)}</Text></View>
            </>
          )}
        </View>
        ...
        {isDraft && (<><View style={{ marginVertical: 8 }}><Button backgroundColor={COLORS.primaryThemeColor} title="Confirm Sale" onPress={handleConfirm} loading={actionLoading} /></View><View style={{ marginVertical: 4 }}><Button backgroundColor="#F44336" title="Cancel" onPress={handleCancel} loading={actionLoading} /></View></>)}
