...
const MobileRepairDetails = ({ navigation, route }) => {
  ...
  const openEstimate = async () => {
    ...
    try {
      ...
      setEstimateData({
        ...
        parts_cost: String(d.parts_cost ?? '0.000'),
  ...
  const updateEstimateField = (field, value) => {
    setEstimateData(prev => {
      ...
      updated.labor_cost = laborCost.toFixed(3);
      updated.total_estimated_cost = (laborCost + partsCost).toFixed(3);
      return updated;
    });
  };
            ...
            <View style={styles.diagTwoCol}>
              ...
              <View style={styles.diagCol}>
                ...
                <View style={styles.diagEstRow}>
                  ...
                  <Text style={styles.diagEstValue}>{parseFloat(data?.estimated_hours || 0).toFixed(3)}</Text>
                </View>
                <View style={styles.diagEstRow}>
                  <Text style={styles.diagEstLabel}>Estimated Parts Cost</Text>
                  <Text style={styles.diagEstValue}>{parseFloat(data?.estimated_parts_cost || 0).toFixed(3)}</Text>
                </View>
                <View style={styles.diagEstRow}>
                  <Text style={styles.diagEstLabel}>Estimated Labor Cost</Text>
                  <Text style={styles.diagEstValue}>{parseFloat(data?.estimated_labor_cost || 0).toFixed(3)}</Text>
                </View>
                <View style={[styles.diagEstRow, styles.diagEstTotal]}>
                  ...
                  <Text style={[styles.diagEstValue, { fontFamily: FONT_FAMILY.urbanistBold, fontSize: 15 }]}>{parseFloat(data?.total_estimated_cost || 0).toFixed(3)}</Text>
                </View>
              </View>
            </View>
                    ...
                    {diagResults.map((dr) => {
                      ...
                      return (
                        <View key={dr.id} style={styles.diagTableRow}>
                          ...
                          <Text style={[styles.diagTableCell, { width: 90, textAlign: 'right', fontFamily: FONT_FAMILY.urbanistBold }]}>{parseFloat(dr.ai_confidence || 0).toFixed(3)}</Text>
                ...
                <>
                  ...
                  {suggestedParts.map((sp) => (
                    <View key={sp.id} style={styles.tableRow}>
                      ...
                      <Text style={[styles.tableCell, { flex: 1, textAlign: 'right' }]}>{(sp.estimated_cost || 0).toFixed(3)}</Text>
          ...
          <View style={styles.tabContent}>
              ...
              <View style={{ minWidth: 700 }}>
                ...
                {serviceLines.map((line) => (
                  <View key={line.id} style={styles.diagTableRow}>
                    ...
                    <Text style={[styles.diagTableCell, { width: 80, textAlign: 'right' }]}>{line.unit_price.toFixed(3)}</Text>
                    <Text style={[styles.diagTableCell, { width: 80, textAlign: 'right', fontWeight: '600' }]}>{line.subtotal.toFixed(3)}</Text>
                    <Text style={[styles.diagTableCell, { width: 70, textAlign: 'right', color: '#666' }]}>{line.tax_amount.toFixed(3)}</Text>
            ...
            <View style={{ borderTopWidth: 1, borderTopColor: '#E0E0E0', marginTop: 8, paddingTop: 10 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'flex-end', marginBottom: 4 }}>
                ...
                <Text style={{ fontSize: 14, fontWeight: '600', minWidth: 70, textAlign: 'right' }}>{(data?.total_service_charge || serviceLines.reduce((s, l) => s + l.subtotal, 0)).toFixed(3)}</Text>
              </View>
              <View style={{ flexDirection: 'row', justifyContent: 'flex-end' }}>
                <Text style={{ fontSize: 14, fontWeight: '700', color: '#333', marginRight: 12 }}>Total Amount:</Text>
                <Text style={{ fontSize: 16, fontWeight: '700', minWidth: 70, textAlign: 'right' }}>$ {(data?.total_amount || 0).toFixed(3)}</Text>
              </View>
            </View>
          </View>
        );
            ...
            {serviceSearchLoading ? (
              ...
              serviceProducts.map(p => (
                ...
                >
                  ...
                  <Text style={{ fontSize: 14, fontFamily: FONT_FAMILY.urbanistBold, color: '#333' }}>{p.list_price.toFixed(3)}</Text>
                </TouchableOpacity>
              ))
            ) : (
              <View style={{ alignItems: 'center', paddingVertical: 30 }}>
      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        <View style={styles.content}>
          ...
          <View style={styles.totalRow}>
            ...
            <Text style={styles.totalValue}>$ {parseFloat(data.total_amount || 0).toFixed(3)}</Text>
          </View>
